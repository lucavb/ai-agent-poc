...
import { getConversations, saveConversation, deleteConversation as deleteConversationFromStorage, generateTitle } from '../utils/storage';
...
export function useConversations() {
  ...
  useEffect(() => {
    ...
    if (loaded.length > 0) {
      const mostRecent = loaded.reduce((prev, current) => 
      ...
      const updated = prev.map(conv => {
        if (conv.id === conversationId) {
          const updatedConv = {
            ...
            title: conv.title === 'New Conversation' && messages.length > 0 && messages[0].role === 'user'
